...
| Railway volume backups / PITR | VERIFIED — NONE ACTIVE | Production Railway UI checked 2026-09-24: Backups page states backup creation and PITR are Pro-only for the current plan and shows `No Backups` / `This service's volume does not have any backups.` Therefore no Railway user-configured volume backup or PITR retention currently applies to this Postgres service. This does not by itself establish whether Railway maintains separate provider-internal infrastructure copies outside the customer Backups feature. |
