...
6. UI displays returned frozen-controller `presentation.decision`, but always labels either `ALLOW` or `STOP` as **synthetic DemoSubject only**, and separately lists source IDs outside the analogous run. Source details are in the HAWM snapshot and existing complete Markdown audit report.
