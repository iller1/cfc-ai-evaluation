...
SUBJECT_MODES = ("SAME_SUBJECT", "OTHER_SUBJECT")


def _install_identity(c: Controller, *, tag: str, label: str, surface: str, entity: str) -> str:
    identity_id = f"id:subject:{tag}:{label}:v1"
    ...
    c.install_verified_identity(
        identity,
        IdentityAuthorityAttestation(
            f"att:subject:{tag}:{label}:identity",
...
def build_case(relation_mode: str, subject_mode: str) -> dict:
    tag = f"{relation_mode.lower()}:{subject_mode.lower()}"
    ...
    demo_identity_id = _install_identity(
        c,
        tag=tag,
        label="demo",
        surface="DemoSubject",
        entity="entity:demo-subject",
    )
    other_identity_id = _install_identity(
        c,
        tag=tag,
        label="other",
        surface="OtherSubject",
        entity="entity:other-subject",
    )
    ...
    for idx, validity in ((1, "CURRENT"), (2, "STALE")):
        eid = f"E{idx}:{tag}"
        source_token = f"subject:{tag}:e{idx}"

        if idx == 2 and subject_mode == "OTHER_SUBJECT":
            surface_subject = "OtherSubject"
            entity_id = "entity:other-subject"
        ...
        evidence = c.draft_evidence_record(
            ...
            authority_record_entity_id=entity_id,
    ...
    scope_id = f"scope:subject:{tag}"
    snapshot = c.draft_snapshot(
        records,
        scope_id=scope_id,
        snapshot_id=f"snapshot:subject:{tag}",
    ...
    return {
        ...
        "e2_subject": (
            "OtherSubject" if subject_mode == "OTHER_SUBJECT" else "DemoSubject"
        ),
...
def run_isolated(relation_mode: str, subject_mode: str) -> dict:
    cp = subprocess.run(
        [
            sys.executable,
            "-m",
            "research.review_stale_relation_subject_relevance",
            "--single-relation",
            relation_mode,
            "--single-subject-mode",
            subject_mode,
...
def main() -> dict:
    ...
    for relation in RELATION_MODES:
        for subject_mode in SUBJECT_MODES:
            rows.append(run_isolated(relation, subject_mode))
    ...
    for relation in RELATION_MODES:
        same = next(
            ...
            and r["subject_mode"] == "SAME_SUBJECT"
        )
        other = next(
            r for r in rows
            if r["relation_mode"] == relation
            and r["subject_mode"] == "OTHER_SUBJECT"
        )
        comparisons[relation] = {
            "same_subject": {
                "claim_state": same["claim_state"],
                "control_closure": same["control_closure"],
                "false_gates": same["false_gates"],
                "global_consistency_violations": same[
                    "global_consistency_violations"
                ],
            },
            "other_subject": {
                "claim_state": other["claim_state"],
                "control_closure": other["control_closure"],
                "false_gates": other["false_gates"],
                "global_consistency_violations": other[
                    "global_consistency_violations"
                ],
            },
            "same_claim_state": same["claim_state"] == other["claim_state"],
            "same_control_closure": same["control_closure"] == other["control_closure"],
            "same_false_gates": same["false_gates"] == other["false_gates"],
        }

    result = {
        "controller_anchor": "0.2.90rc1",
        "test": "STALE_RELATION_SUBJECT_RELEVANCE",
        ...
        "interpretation_boundary": (
            "E1 is POSITIVE/CURRENT for DemoSubject and alone satisfies required "
            "supports=1. E2 is POSITIVE/STALE. Both identities are installed in every "
            "case. E2 either remains about DemoSubject or becomes a fully bound record "
            "about OtherSubject. One shared provenance/dependency relation is held "
            "between E1 and E2 per case."
        ),
    }
    Path("stale_relation_subject_relevance.json").write_text(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--single-subject-mode", choices=SUBJECT_MODES)
    args = parser.parse_args()
    if args.single_relation:
        if args.single_subject_mode is None:
            raise SystemExit("--single-subject-mode is required")
        print(json.dumps(
            build_case(args.single_relation, args.single_subject_mode),
            sort_keys=True,
        ))
    else:
        main()
