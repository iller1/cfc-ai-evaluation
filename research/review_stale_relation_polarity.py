...
STALE_POLARITIES = ("POSITIVE", "NEGATIVE")


def build_case(relation_mode: str, stale_polarity: str) -> dict:
    tag = f"{relation_mode.lower()}:{stale_polarity.lower()}"
    ...
    identity_id = f"id:polarity:{tag}:subject:v1"
    ...
    records = []
    for idx, validity, polarity in (
        (1, "CURRENT", "POSITIVE"),
        (2, "STALE", stale_polarity),
    ):
        eid = f"E{idx}:{tag}"
        source_token = f"polarity:{tag}:e{idx}"
    ...
    scope_id = f"scope:polarity:{tag}"
    snapshot = c.draft_snapshot(
        records,
        scope_id=scope_id,
        snapshot_id=f"snapshot:polarity:{tag}",
...
def run_isolated(relation_mode: str, polarity: str) -> dict:
    cp = subprocess.run(
        [
            sys.executable,
            "-m",
            "research.review_stale_relation_polarity",
            "--single-relation",
            relation_mode,
            "--single-polarity",
            polarity,
...
def main() -> dict:
    ...
    for relation in RELATION_MODES:
        for polarity in STALE_POLARITIES:
            rows.append(run_isolated(relation, polarity))
    ...
    for relation in RELATION_MODES:
        pos = next(
            ...
            and r["stale_polarity"] == "POSITIVE"
        )
        neg = next(
            r for r in rows
            if r["relation_mode"] == relation
            and r["stale_polarity"] == "NEGATIVE"
        )
        comparisons[relation] = {
            "positive": {
                "claim_state": pos["claim_state"],
                "control_closure": pos["control_closure"],
                "false_gates": pos["false_gates"],
                "global_consistency_violations": pos[
                    "global_consistency_violations"
                ],
            },
            "negative": {
                "claim_state": neg["claim_state"],
                "control_closure": neg["control_closure"],
                "false_gates": neg["false_gates"],
                "global_consistency_violations": neg[
                    "global_consistency_violations"
                ],
            },
            "same_claim_state": pos["claim_state"] == neg["claim_state"],
            "same_control_closure": pos["control_closure"] == neg["control_closure"],
            "same_false_gates": pos["false_gates"] == neg["false_gates"],
        }

    result = {
        "controller_anchor": "0.2.90rc1",
        "test": "STALE_RELATION_POLARITY",
        "state_count": len(rows),
        "relations": list(RELATION_MODES),
        "stale_polarities": list(STALE_POLARITIES),
        "comparisons": comparisons,
        "rows": rows,
        "interpretation_boundary": (
            "E1 is POSITIVE/CURRENT and alone satisfies the one-support claim policy. "
            "E2 is STALE and differs only in polarity. One shared relation is present "
            "per case. This tests whether stale-relation authorization persistence "
            "depends on stale evidence polarity."
        ),
    }
    Path("stale_relation_polarity.json").write_text(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--single-polarity", choices=STALE_POLARITIES)
    args = parser.parse_args()
    if args.single_relation:
        print(json.dumps(
            build_case(args.single_relation, args.single_polarity),
            sort_keys=True,
        ))
    else:
        main()
