...
PREDICATE_MODES = ("MATCHED", "OTHER_PREDICATE")


def build_case(relation_mode: str, predicate_mode: str) -> dict:
    tag = f"{relation_mode.lower()}:{predicate_mode.lower()}"
    ...
    identity_id = f"id:predicate:{tag}:subject:v1"
    ...
    for idx, validity in ((1, "CURRENT"), (2, "STALE")):
        eid = f"E{idx}:{tag}"
        source_token = f"predicate:{tag}:e{idx}"
        ...
        predicate = (
            "other_state"
            if idx == 2 and predicate_mode == "OTHER_PREDICATE"
            else "state"
        )
    ...
    scope_id = f"scope:predicate:{tag}"
    snapshot = c.draft_snapshot(
        records,
        scope_id=scope_id,
        snapshot_id=f"snapshot:predicate:{tag}",
    ...
    return {
        ...
        "e2_predicate": (
            "other_state" if predicate_mode == "OTHER_PREDICATE" else "state"
        ),
...
def run_isolated(relation_mode: str, predicate_mode: str) -> dict:
    cp = subprocess.run(
        [
            sys.executable,
            "-m",
            "research.review_stale_relation_predicate_relevance",
            "--single-relation",
            relation_mode,
            "--single-predicate-mode",
            predicate_mode,
        ],
        capture_output=True,
        text=True,
...
def main() -> dict:
    ...
    for relation in RELATION_MODES:
        for predicate_mode in PREDICATE_MODES:
            rows.append(run_isolated(relation, predicate_mode))
    ...
    for relation in RELATION_MODES:
        matched = next(
            ...
            and r["predicate_mode"] == "MATCHED"
        )
        other = next(
            r for r in rows
            if r["relation_mode"] == relation
            and r["predicate_mode"] == "OTHER_PREDICATE"
        )
        comparisons[relation] = {
            "matched": {
                "claim_state": matched["claim_state"],
                "control_closure": matched["control_closure"],
                "false_gates": matched["false_gates"],
                "global_consistency_violations": matched[
                    "global_consistency_violations"
                ],
            },
            "other_predicate": {
            ...
            "same_claim_state": matched["claim_state"] == other["claim_state"],
            "same_control_closure": (
                matched["control_closure"] == other["control_closure"]
            ),
            "same_false_gates": matched["false_gates"] == other["false_gates"],
        }

    result = {
        "controller_anchor": "0.2.90rc1",
        "test": "STALE_RELATION_PREDICATE_RELEVANCE",
        "state_count": len(rows),
        "relations": list(RELATION_MODES),
        "predicate_modes": list(PREDICATE_MODES),
        "comparisons": comparisons,
        "rows": rows,
        "interpretation_boundary": (
            "E1 is POSITIVE/CURRENT and alone satisfies required supports=1. "
            "E2 is POSITIVE/STALE. One shared provenance/dependency relation is "
            "present per case. Only E2 predicate changes between the claim-matched "
            "predicate 'state' and an unrelated predicate 'other_state'."
        ),
    }
    Path("stale_relation_predicate_relevance.json").write_text(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--single-predicate-mode", choices=PREDICATE_MODES)
    args = parser.parse_args()
    if args.single_relation:
        if args.single_predicate_mode is None:
            raise SystemExit("--single-predicate-mode is required")
        print(json.dumps(
            build_case(args.single_relation, args.single_predicate_mode),
            sort_keys=True,
        ))
    else:
        main()
