...
class FrontendTests(unittest.TestCase):
    ...
    def test_dedicated_founding_beta_page_is_content_free(self):
        ...
        self.assertIn('id="fb-delete-measurements"', body)
    ...
    def test_dedicated_founding_beta_js_uses_ephemeral_check_and_measurements(self):
        ...
        self.assertIn("deleted_measurements", body)
    ...
    def test_attachment_picker_is_limited_to_text_and_discloses_persistence(self):
        ...
        self.assertIn('accept=".txt,.md,.pdf,.docx,', body)
        ...
        self.assertIn('id="attachment-review"', body)
        self.assertIn('id="attachment-confirm"', body)
    ...
    def test_message_copy_and_text_attachment_are_not_cfc_authorizations(self):
        ...
        self.assertIn('DOCUMENT_MAX_BYTES = 2 * 1024 * 1024', js)
        self.assertIn('/extract-document",', js)
        self.assertIn('result.status !== "EXTRACTED_TEXT_UNVERIFIED"', js)
        self.assertIn('pendingAttachment.requiresReview && !attachmentConfirm.checked', js)
    ...
    def test_review_bridge_uses_explicit_human_scope_and_never_automatically_verifies_model(self):
        ...
        self.assertIn("ProBetaReviewBridge.buildReview(review)", js)
        self.assertIn('"/api/conversations/" + conversationId + "/reviewed-demo-scope"', js)
        self.assertIn("JSON.stringify({review, working_state: workingState})", js)
        self.assertIn("savedReviewSnapshot.review_manifest.server_validation", js)
        self.assertIn('savedReviewSnapshot.cfc_structured?.independence_authority !== "NONE"', js)
        self.assertNotIn("state.review_manifest = prepared.review_manifest", js)
    ...
    def test_benchmark_history_ui_is_present(self):
        status, body, _ = self.get("/")
        self.assertEqual(status, 200)
        self.assertIn('id="benchmark-history"', body)
        self.assertIn('id="refresh-benchmark-history"', body)
        self.assertIn("No automatic semantic PASS/FAIL", body)

    def test_app_js_loads_persisted_benchmark_history(self):
        with patch.dict(
            os.environ,
            {
                "CLERK_PUBLISHABLE_KEY": "pk_test_example",
            ...
            status, body, _ = self.get("/app.js")
        self.assertEqual(status, 200)
        self.assertIn("/benchmark-runs", body)
        self.assertIn("Automatic semantic scoring: disabled", body)
    ...
    def test_app_js_contains_benchmark_scoreboard_aggregation(self):
        ...
        self.assertIn('row.benchmark_version || "UNKNOWN_BENCHMARK_VERSION"', body)
        self.assertIn("benchmark_version: benchmarkVersion", body)
        self.assertIn('"Benchmark version"', body)
        self.assertIn('[a.benchmark_version, a.case_id, a.provider, a.model]', body)
