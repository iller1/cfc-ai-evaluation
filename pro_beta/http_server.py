...
class ProBetaHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "conversations" and parts[3] == "benchmark-runs":
            credential = _bearer(self.headers)
            if not credential:
                self._json(HTTPStatus.UNAUTHORIZED, {"error": "AUTH_CREDENTIAL_REQUIRED"})
                return
            ...
            self._api_call(
                lambda api: api.list_benchmark_runs(
    ...
    def do_POST(self) -> None:
        ...
        parts = [part for part in path.split("/") if part]
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "workspaces" and parts[3] == "beta-cfc-check":
            if not credential:
                self._json(HTTPStatus.UNAUTHORIZED, {"error": "AUTH_CREDENTIAL_REQUIRED"})
                return
            payload = self._payload()
            workspace_id = parts[2]
            self._api_call(
                lambda api: api.run_founding_beta_structured_cfc(
        ...
        if len(parts) == 4 and parts[0] == "api" and parts[1] == "conversations" and parts[3] == "openai-chat":
            ...
            conversation_id = parts[2]
            self._api_call(
                lambda api: api.chat_with_openai(
                    credential,
                    conversation_id,
                    payload,
                )
