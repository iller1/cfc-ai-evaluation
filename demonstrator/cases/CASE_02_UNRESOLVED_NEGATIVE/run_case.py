...
ASOF = "2026-09-03"
AUTHORITIES = {
    "IDENTITY": "DEMO_CASE02_IDENTITY_AUTHORITY",
    "SOURCE_SEMANTICS": "DEMO_CASE02_SOURCE_SEMANTICS_AUTHORITY",
    "PROVENANCE": "DEMO_CASE02_PROVENANCE_AUTHORITY",
    "EVIDENCE_AUTHORITY": "DEMO_CASE02_EVIDENCE_AUTHORITY",
    "EPISTEMIC_ROLE": "DEMO_CASE02_EPISTEMIC_ROLE_AUTHORITY",
    "RETRIEVAL": "DEMO_CASE02_RETRIEVAL_AUTHORITY",
    "FAILURE_DOMAIN_TOPOLOGY": "DEMO_CASE02_FAILURE_DOMAIN_TOPOLOGY_AUTHORITY",
}
...
class IdentityVerifier:
    def verify(self, a, *, draft):
        return IdentityAuthorityVerdict(True, "demo-case02:id:v1", a.attestation_id, a.authority_id, "SYNTHETIC_CASE_FIXTURE")
class SourceSemanticsVerifier:
    def verify(self, a, *, draft):
        return SourceSemanticsAuthorityVerdict(True, "demo-case02:ss:v1", a.attestation_id, a.authority_id, "SYNTHETIC_CASE_FIXTURE")
class ProvenanceVerifier:
    def verify(self, a, *, draft):
        return ProvenanceAuthorityVerdict(True, "demo-case02:prov:v1", a.attestation_id, a.authority_id, a.evidence_id, "SYNTHETIC_CASE_FIXTURE")
class EvidenceVerifier:
    def verify(self, a, *, draft):
        return EvidenceAuthorityVerdict(True, "demo-case02:ev:v1", a.attestation_id, a.authority_id, a.evidence_id, "SYNTHETIC_CASE_FIXTURE")
class RoleVerifier:
    def verify(self, a, *, draft, evidence):
        return EpistemicRoleAuthorityVerdict(True, "demo-case02:role:v1", a.attestation_id, a.authority_id, a.evidence_id, a.epistemic_role, "SYNTHETIC_CASE_FIXTURE")
class RetrievalVerifier:
    def verify(self, a, *, draft, evidence):
        return RetrievalAuthorityVerdict(True, "demo-case02:ret:v1", a.attestation_id, a.authority_id, "SYNTHETIC_CASE_FIXTURE")
class TopologyVerifier:
    def verify(self, a, *, draft):
        return FailureDomainTopologyVerdict(True, "demo-case02:topo:v1", a.attestation_id, a.authority_id, a.topology_commitment, "SYNTHETIC_CASE_FIXTURE")
...
c.install_verified_identity(
    identity,
    IdentityAuthorityAttestation("att:case02:identity", AUTHORITIES["IDENTITY"], c.identity_commitment(identity), ASOF, "2026-01-01", "2026-12-31"),
    verifiers["IDENTITY"], as_of=ASOF,
)

semantics = c.draft_source_semantics(
    semantics_registry_entry_id="sem:case02:e1", source_id="general-record:case02:e1",
    repository_id="repo:case02", producer_id="producer:case02", process_id="process:case02",
    failure_domain_id="fd:case02:e1", resolution_state="KNOWN",
)
c.install_verified_source_semantics(
    semantics,
    SourceSemanticsAuthorityAttestation("att:case02:semantics", AUTHORITIES["SOURCE_SEMANTICS"], c.source_semantics_commitment(semantics), ASOF, "2026-01-01", "2026-12-31"),
...
c.install_verified_failure_domain_topology(
    topology,
    FailureDomainTopologyAttestation("att:case02:topology", AUTHORITIES["FAILURE_DOMAIN_TOPOLOGY"], c.failure_domain_topology_commitment(topology), ASOF, "2026-01-01", "2026-12-31"),
...
dependencies = {k: {"state":"KNOWN", "id":f"{v}:case02:e1"} for k, v in prefix.items()}
provenance = {
    "source_id":"general-record:case02:e1",
    "root_origin_id":"general-record:root:case02:e1",
    "origin_id":"general-record:origin:case02:e1",
    "referent_entity_id":"entity:demo-subject",
    "referent_event_id":"event:current",
    "referent_version_id":"v1",
    "extractor_id":"extractor:case02:e1",
    "common_mode_group":"group:case02:e1",
    "lineage":["general-record:root:case02:e1", "general-record:origin:case02:e1"],
    "dependencies":dependencies,
}
evidence = c.draft_evidence_record(
    evidence_id="E1", subject="DemoSubject", predicate="state", value="safe",
    source="display:case02:e1", identity_registry_entry_id="id:demo-subject:v1",
    ...
    valid_from="2026-01-01", valid_to="2026-12-31", observed_at="2026-12-01", available_at="2026-12-01",
    provenance=provenance, polarity="POSITIVE", source_semantics_id="sem:case02:e1",
)
c.verify_evidence_provenance(
    evidence,
    ProvenanceAuthorityAttestation("att:case02:provenance", AUTHORITIES["PROVENANCE"], "E1", c.provenance_commitment(evidence), ASOF, "2026-01-01", "2026-12-31"),
    verifiers["PROVENANCE"], as_of=ASOF,
)
c.verify_evidence_authority(
    evidence,
    EvidenceAuthorityAttestation("att:case02:evidence", AUTHORITIES["EVIDENCE_AUTHORITY"], "E1", "GENERAL_RECORD_V5", c.evidence_authority_commitment(evidence), ASOF, "2026-01-01", "2026-12-31"),
...
role_installation = c.install_verified_epistemic_role(
    evidence, role,
    EpistemicRoleAuthorityAttestation("att:case02:role", AUTHORITIES["EPISTEMIC_ROLE"], "E1", "DIRECT_WORLD_RECORD", role.role_commitment, ASOF, "2026-01-01", "2026-12-31"),
...
snapshot = c.draft_snapshot(
    [record], scope_id="scope:demo:case02", snapshot_id="snapshot:demo:case02",
    snapshot_created_at=ASOF,
    snapshot_available_at=ASOF, valid_from="2026-01-01", valid_to="2026-12-31",
)
c.install_verified_snapshot(
    snapshot, [record],
    RetrievalAuthorityAttestation("att:case02:retrieval", AUTHORITIES["RETRIEVAL"], c.snapshot_commitment(snapshot), ASOF, "2026-01-01", "2026-12-31"),
    verifiers["RETRIEVAL"], as_of=ASOF,
)

result = c.evaluate_snapshot(
    snapshot, "DemoSubject is not safe.", [record], {"c1":"id:demo-subject:v1"}, as_of=ASOF, requirements={"c1":{"required_independent_supports":1}},
)
summary = {
    "case_id":"CASE_02_UNRESOLVED_NEGATIVE",
    "engine_sha256":c.controller_sha256,
    "mandatory_gate_count":c.mandatory_gate_count,
    "persistence_schema":c.persistence_schema,
    "model_conclusion":"DemoSubject is not safe.",
    "evidence":[record],
