...
c.install_verified_identity(
    identity,
    IdentityAuthorityAttestation("att:case09:identity", AUTHORITIES["IDENTITY"], c.identity_commitment(identity), ASOF, "2026-01-01", "2026-12-31"),
...
c.install_verified_failure_domain_topology(
    topology,
    FailureDomainTopologyAttestation("att:case09:topology", AUTHORITIES["FAILURE_DOMAIN_TOPOLOGY"], c.failure_domain_topology_commitment(topology), ASOF, "2026-01-01", "2026-12-31"),
...
def trusted_record(eid: str, polarity: str):
    sem_id=f"sem:case09:{eid.lower()}"
    source_id=f"general-record:case09:{eid.lower()}"
    semantics=c.draft_source_semantics(
        semantics_registry_entry_id=sem_id, source_id=source_id,
        repository_id=f"repo:case09:{eid.lower()}", producer_id=f"producer:case09:{eid.lower()}", process_id=f"process:case09:{eid.lower()}",
        failure_domain_id=f"fd:case09:{eid.lower()}", resolution_state="KNOWN",
    )
    c.install_verified_source_semantics(
        semantics,
        SourceSemanticsAuthorityAttestation(f"att:case09:semantics:{eid}", AUTHORITIES["SOURCE_SEMANTICS"], c.source_semantics_commitment(semantics), ASOF, "2026-01-01", "2026-12-31"),
        verifiers["SOURCE_SEMANTICS"], as_of=ASOF,
    )
    deps={k:{"state":"KNOWN", "id":f"{v}:case09:{eid.lower()}"} for k,v in prefix.items()}
    provenance={
        "source_id":source_id,
        "root_origin_id":f"general-record:root:case09:{eid.lower()}",
        "origin_id":f"general-record:origin:case09:{eid.lower()}",
        "referent_entity_id":"entity:demo-subject", "referent_event_id":"event:current", "referent_version_id":"v1",
        "extractor_id":f"extractor:case09:{eid.lower()}", "common_mode_group":f"group:case09:{eid.lower()}",
        "lineage":[f"general-record:root:case09:{eid.lower()}", f"general-record:origin:case09:{eid.lower()}"],
        "dependencies":deps,
    }
    draft=c.draft_evidence_record(
        evidence_id=eid, subject="DemoSubject", predicate="state", value="safe", source=f"display:case09:{eid.lower()}",
    ...
    c.verify_evidence_provenance(
        draft,
        ProvenanceAuthorityAttestation(f"att:case09:provenance:{eid}", AUTHORITIES["PROVENANCE"], eid, c.provenance_commitment(draft), ASOF, "2026-01-01", "2026-12-31"),
        verifiers["PROVENANCE"], as_of=ASOF,
    )
    c.verify_evidence_authority(
        draft,
        EvidenceAuthorityAttestation(f"att:case09:evidence:{eid}", AUTHORITIES["EVIDENCE_AUTHORITY"], eid, "GENERAL_RECORD_V5", c.evidence_authority_commitment(draft), ASOF, "2026-01-01", "2026-12-31"),
    ...
    role_install=c.install_verified_epistemic_role(
        draft, role,
        EpistemicRoleAuthorityAttestation(f"att:case09:role:{eid}", AUTHORITIES["EPISTEMIC_ROLE"], eid, "DIRECT_WORLD_RECORD", role.role_commitment, ASOF, "2026-01-01", "2026-12-31"),
...
snapshot=c.draft_snapshot(
    records, scope_id="scope:demo:case09", snapshot_id="snapshot:demo:case09", snapshot_created_at=ASOF, snapshot_available_at=ASOF, valid_from="2026-01-01", valid_to="2026-12-31",
)
c.install_verified_snapshot(
    snapshot, records,
    RetrievalAuthorityAttestation("att:case09:retrieval", AUTHORITIES["RETRIEVAL"], c.snapshot_commitment(snapshot), ASOF, "2026-01-01", "2026-12-31"),
...
SCOPE="scope:demo:case09"

# E1 + E2 are the selected independent supports.
ssi=c.draft_support_set_independence(
    independence_id="ssi:case09:selected", claim_id="c1", retrieval_scope_id=SCOPE, evidence_ids=["E1","E2"],
...
ssi_att=SupportSetIndependenceAuthorityAttestation(
    "att:case09:ssi", AUTHORITIES["SUPPORT_SET_INDEPENDENCE"], c.support_set_independence_commitment(ssi,drafts[:2]), ASOF, "2026-01-01", "2026-12-31"
)
...
relation=c.draft_support_relation(
    relation_id="rel:case09:e2-e3-redundancy", relation_type="COMMON_MODE", evidence_ids=["E2","E3"],
...
rel_att=SupportRelationAuthorityAttestation(
    "att:case09:support-relation", AUTHORITIES["SUPPORT_RELATION"], c.support_relation_commitment(relation,drafts[1:]), ASOF, "2026-01-01", "2026-12-31"
)
...
selection=c.draft_support_selection(
    selection_id="sel:case09", claim_id="c1", retrieval_scope_id=SCOPE,
...
sel_att=SupportSelectionAuthorityAttestation(
    "att:case09:support-selection", AUTHORITIES["SUPPORT_SELECTION"], c.support_selection_commitment(selection,drafts), ASOF, "2026-01-01", "2026-12-31"
)
...
resolution=c.draft_relation_resolution(
    resolution_id="rr:case09:e2-e3", relation_id="rel:case09:e2-e3-redundancy", claim_id="c1", retrieval_scope_id=SCOPE,
...
rr_att=RelationResolutionAuthorityAttestation(
    "att:case09:relation-resolution", AUTHORITIES["RELATION_RESOLUTION"], c.relation_resolution_commitment(resolution,drafts), ASOF, "2026-01-01", "2026-12-31"
)
rr_install=c.install_verified_relation_resolution(resolution,drafts,rr_att,verifiers["RELATION_RESOLUTION"],text=TEXT,claim_identity_map=CLAIM_MAP,as_of=ASOF,requirements=REQ)

disposition=c.draft_excluded_support_disposition(
    disposition_id="disp:case09:e3", claim_id="c1", retrieval_scope_id=SCOPE, excluded_evidence_id="E3",
    selected_evidence_ids=["E1","E2"], supporting_relation_ids=["rel:case09:e2-e3-redundancy"],
...
disp_att=ExcludedSupportDispositionAuthorityAttestation(
    "att:case09:excluded-disposition", AUTHORITIES["EXCLUDED_SUPPORT_DISPOSITION"], c.excluded_support_disposition_commitment(disposition,drafts), ASOF, "2026-01-01", "2026-12-31"
)
...
# Deliberately do NOT finalize/bind the verified resolution to the exact decision context.
# The frozen public API documents install_verified_relation_resolution() as PENDING until
# finalize_verified_decision_authorities_for_prepare() succeeds. An unbound declaration
# must therefore not authorize closure.
bound=tuple()
result=c.evaluate_snapshot(snapshot,TEXT,records,CLAIM_MAP,as_of=ASOF,requirements=REQ)
summary={
    "case_id":"CASE_09_INACTIVE_UNBOUND_RESOLUTION",
    ...
    "resolution_install_runtime_verified":rr_install.runtime_verified,
    "resolution_install_bound":rr_install.bound,
